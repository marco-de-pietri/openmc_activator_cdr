...
"""
Download TENDL 2019/2017/2015 ACE files from TENDL mirrors and
...
from openmc_data_utils import download, extract
...
if args.download:
    ...
    for f in release_details[args.release]["compressed_files"]:
        ...
        download(
            ...
            as_browser=release_details[args.release].get("as_browser", False),
        )
...
if args.extract:
    for f in release_details[args.release]["compressed_files"]:
        extract(
            compressed_file=download_path / f,
...
neutron_files = sorted(ace_files_dir.glob(release_details[args.release]["neutron_files"]))
if not neutron_files:
    raise FileNotFoundError(
        "No ACE neutron files were found after download/extraction. "
        f"Expected files matching {ace_files_dir / release_details[args.release]['neutron_files']}"
    )
